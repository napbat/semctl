...
- **read_source** — focused revision-pinned line/byte ranges from remote-only or otherwise unavailable content; use host Read for current bytes at a known local path.
...
Every codebase-scoped tool accepts an optional codebase selector. Omit it for the
launch/current repository. To work across repositories, pass either an immutable
codebase id or an indexed local directory path. A previously indexed local
repository already has consent for retrieval and watcher activation, so use it
without asking again; that consent does not authorize edits beyond the user's
...
4. Understand how files depend on each other → `imports`, then `symbol_edges` for
   the exact bindings.
5. A dependency points outside this codebase → `external_links`.
6. Have a known indexed file or a hit's line range → `file_outline` for its shape,
   `expand_chunk` to grow focused context. Use host Read for current local bytes.
7. Need exact remote/revision-pinned bytes → `read_source`; need a refactor → call
   the matching edit action, which obtains the server plan internally and applies
   it after the host's normal approval.

Prefer the symbol-graph tools over search whenever you have an exact name — they're cheaper and more precise. Symbol-graph coverage follows the language packs registered on the server; Rust, C#, Go, TypeScript/JavaScript, and C++ resolve on the current service. C++ needs build context in the checkout (a CMakeLists.txt or a compile database) and can still miss member declarations and member calls, so cross-check an empty C++ answer with `grep`. For a file in a language without a registered pack the graph tools return nothing, so use `search_codebase` / `grep` there.
